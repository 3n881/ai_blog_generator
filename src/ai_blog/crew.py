...
class AiBlog():
	...
	@agent
	def retrieve_blog(self) -> Agent:
		return Agent(
			config=self.agents_config['retrieve_blog'],
			tools=[SerperDevTool()],
			verbose=True
		)
	...
	@task
	def retrieve_blog_task(self) -> Task:
		return Task(
			config=self.tasks_config['retrieve_blog_task'],
		)
